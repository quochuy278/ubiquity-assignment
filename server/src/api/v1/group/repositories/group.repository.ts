...
import type { CreateGroupInput, GroupResult } from '../group.types';
...
export class GroupRepository {
  ...
  async findByIds(groupIds: readonly string[]): Promise<GroupResult[]> {
  ...
  async findById(groupId: string): Promise<GroupResult | null> {
    const group = await this.prisma.group.findUnique({ where: { id: groupId } });

    return group ? this.toGroupResult(group) : null;
  }

  private toGroupResult(group: Group): GroupResult {
